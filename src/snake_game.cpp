...
int main(void)
{
    ...
    SetTargetFPS(60); ///numarul cadrelor pe secunda poate fi modificat pentru a face sarpele si implicit jocul mai lent/rapid
...
// functia de update al jocului (apelata la fiecare cadru)
void UpdateGame(void)
{
    if (!gameOver)
    {
        ...
        if (!pause)
        {
            ...
            // comportamentul la ciocnirea de un zid
            ...
            // comportamentul la ciocnirea cu sinele (sarpele)
            for (int i = 1; i < counterTail; i++)
            {
                if ((snake[0].position.x == snake[i].position.x) && (snake[0].position.y == snake[i].position.y)) gameOver = true;
            }

            // calcularea pozitiei fructelor
            if (!fruit.active)
            {
                fruit.active = true;
...
void DrawGame(void)
{
    BeginDrawing();
        ...
        if (!gameOver)
        {
            // deseneaza liniile si coloanele graficului din background
